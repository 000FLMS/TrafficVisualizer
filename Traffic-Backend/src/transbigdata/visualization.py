...
def visualization_od(oddata, col=['slon', 'slat', 'elon', 'elat'],
                     zoom='auto', height=500, accuracy=500, mincount=0):
    '''
    The input is the OD data and the column. The output is the visualization
    result based on kepler
    ...
    col : List
        ...
        (OD) data, in the sequence of [origin longitude, origin latitude,
        destination longitude, destination latitude]. For this, The aggregation
        is automatic. Or, the user can also input a weighted OD data, in the
        sequence of [origin longitude, origin latitude, destination longitude,
        destination latitude, count]
    zoom : number
